...
final class ECP2 {
    ...
    func equals(Q:ECP2) -> Bool
    {
        ...
        var zs2=FP2(z); zs2.sqr()
        var zo2=FP2(Q.z); zo2.sqr()
        var zs3=FP2(zs2); zs3.mul(z)
        var zo3=FP2(zo2); zo3.mul(Q.z)
        zs2.mul(Q.x)
        zo2.mul(x)
...
/* set self*=q, where q is Modulus, using Frobenius */
    ...
    /* make exponent odd - add 2P if even, P if odd */
        t.copy(e)
        var s=t.parity()
        t.inc(1); t.norm(); var ns=t.parity(); mt.copy(t); mt.inc(1); mt.norm()
        t.cmove(mt,s)
        Q.cmove(self,ns)
        C.copy(Q)
